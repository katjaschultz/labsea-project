...
- filename: The input file containing the profiles (Argo or CTD).
- mask_profiles: A NumPy array of profile indices to be used for filtering (set to 'empty' if all profiles should be included in the composite).
- folder_path: The path to the folder where the output file will be saved.
- spacing_z: Vertical grid spacing in meters (default is 25).
- spacing_x: Horizontal grid spacing in kilometers (default is 10).
- omega: Gaussian weighting parameter (default is 30.0).
- xstart: Starting x-coordinate in kilometers (default is 200).
- xend: Ending x-coordinate in kilometers (default is 860).
